describe('Fiscal Year Page Test', () => {
    let tenantName;
    ...
    beforeEach(() => {
        cy.wrap(tenantName).should('not.be.undefined').then((name) => {
            ...
            cy.get('.lpx-nav > lpx-navbar-routes > .lpx-nav-menu > :nth-child(8) > :nth-child(1) > .lpx-inner-menu > :nth-child(2) > lpx-sub-navbar > .lpx-menu-item-link > .lpx-menu-item-text').click(); // Sub-menu for Purchase List
        });
    });
    it('should verify visibility of essential elements on the Fiscal Year Data page', () => {
        // Verify presence of Supplier page header
        cy.get('h3').contains('Fiscal Year Data').should('be.visible'); // Page header
        ...
        cy.get('#abp-modal-header > h3').contains('Create New Fiscal Year').should('be.visible'); // Page header
