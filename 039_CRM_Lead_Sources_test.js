describe('Lead Sources Page Test', () => {
    let tenantName;
    ...
    beforeEach(() => {
        cy.wrap(tenantName).should('not.be.undefined').then((name) => {
            ...
            cy.get('.lpx-nav > lpx-navbar-routes > .lpx-nav-menu > :nth-child(9) > :nth-child(1) > .lpx-inner-menu > :nth-child(1) > lpx-sub-navbar > .lpx-menu-item-link > .lpx-menu-item-text').click(); // Sub-menu for Purchase List
        });
    });


    it('should verify visibility of essential elements on the Lead Sources page', () => {
        // Verify presence of Supplier page header
        cy.get('.card-title').contains('Lead Sources').should('be.visible'); // Page header
        ...
        cy.get('h3').contains('Create New Lead Source').should('be.visible');
